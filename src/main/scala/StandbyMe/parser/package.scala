...
package object parser {
  ...
  def reduce(production: Production, right: Vector[Node]): Node = {
    production match {
      ...
      case (SyntacticSymbol.EXPRESSION, Vector(SyntacticSymbol.EXPRESSION, SyntacticSymbol.EQ, SyntacticSymbol.EXPRESSION)) => BinaryOpNode("==", right(0).asInstanceOf[ExprNode], right(2).asInstanceOf[ExprNode])
    }
  }
